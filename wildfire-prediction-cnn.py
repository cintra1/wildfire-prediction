...
plt.title('CNN - Acurácia do Modelo')
plt.ylabel('Acurácia')
plt.xlabel('Época')
...
plt.title('CNN - Matriz de Confusão - Treinamento')
plt.show()
if Y_pred_train.shape[1] == 2:
    ...
    plt.title('CNN - Curva ROC - Treinamento')
    plt.legend()
    plt.show()
...
print("Classification Report:\n", classification_report(Y_val_enc, Y_pred_val_classes, target_names=le.classes_))
fig, ax = plt.subplots()
ConfusionMatrixDisplay.from_predictions(Y_val_enc, Y_pred_val_classes, display_labels=le.classes_, cmap=plt.cm.Blues, ax=ax)
plt.title('CNN - Matriz de Confusão - Validação')
plt.show()
if Y_pred_val.shape[1] == 2:
    auc_val = roc_auc_score(Y_val_enc, Y_pred_val[:,1])
    print("AUC (Validação):", auc_val)
    fpr, tpr, _ = roc_curve(Y_val_enc, Y_pred_val[:,1])
    plt.plot(fpr, tpr, label=f'Validação (AUC = {auc_val:.2f})')
    plt.plot([0,1],[0,1],'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('CNN - Curva ROC - Validação')
    plt.legend()
    plt.show()
...
plt.title('CNN - Matriz de Confusão - Teste')
plt.show()
...
ax.set_title('CNN - Comparação das principais métricas por conjunto')
ax.set_xticks(x)
...
print("\nCNN - Exemplos de classificação no conjunto de teste:")
num_examples = 5
